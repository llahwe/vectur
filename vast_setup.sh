...
if [[ "${SYNC_GIT}" == "true" ]]; then
  ...
  REMOTE_CMD+=$'# Install uv if not present and sync dependencies from pyproject.toml\n'
  REMOTE_CMD+=$'if ! command -v uv >/dev/null 2>&1; then\n'
  REMOTE_CMD+=$'  echo "[vast_setup][remote] Installing uv..."\n'
  REMOTE_CMD+=$'  curl -LsSf https://astral.sh/uv/install.sh | sh\n'
  REMOTE_CMD+=$'  export PATH="${HOME}/.cargo/bin:${PATH}"\n'
  REMOTE_CMD+=$'fi\n'
  REMOTE_CMD+=$'if command -v uv >/dev/null 2>&1 && [[ -f "${REPO_DIR}/pyproject.toml" ]]; then\n'
  REMOTE_CMD+=$'  echo "[vast_setup][remote] Syncing dependencies with uv..."\n'
  REMOTE_CMD+=$'  uv sync --frozen || uv sync\n'
  REMOTE_CMD+=$'fi\n'
fi
...
if [[ "${LAUNCH_SCHEDULER}" == "true" ]]; then
  ...
  REMOTE_CMD+=$'# Install uv if not present (required for pyproject.toml dependency management)\n'
  REMOTE_CMD+=$'if ! command -v uv >/dev/null 2>&1; then\n'
  REMOTE_CMD+=$'  echo "[vast_setup][remote] Installing uv..."\n'
  REMOTE_CMD+=$'  curl -LsSf https://astral.sh/uv/install.sh | sh\n'
  REMOTE_CMD+=$'  export PATH="${HOME}/.cargo/bin:${PATH}"\n'
  REMOTE_CMD+=$'  if ! command -v uv >/dev/null 2>&1; then\n'
  REMOTE_CMD+=$'    echo "[vast_setup][remote] error: failed to install uv."\n'
  REMOTE_CMD+=$'    exit 1\n'
  REMOTE_CMD+=$'  fi\n'
  REMOTE_CMD+=$'fi\n'
  REMOTE_CMD+=$'# Sync dependencies from pyproject.toml\n'
  REMOTE_CMD+=$'if [[ -f "pyproject.toml" ]]; then\n'
  REMOTE_CMD+=$'  echo "[vast_setup][remote] Syncing dependencies with uv..."\n'
  REMOTE_CMD+=$'  uv sync --frozen || uv sync\n'
  REMOTE_CMD+=$'else\n'
  REMOTE_CMD+=$'  echo "[vast_setup][remote] warning: pyproject.toml not found; skipping uv sync"\n'
  REMOTE_CMD+=$'fi\n'
  REMOTE_CMD+=$'# Use uv run python3 if uv is available, otherwise fall back to plain python3\n'
  REMOTE_CMD+=$'if command -v uv >/dev/null 2>&1 && [[ -f "pyproject.toml" ]]; then\n'
  REMOTE_CMD+=$'  PYBIN="uv run python3"\n'
  REMOTE_CMD+=$'  echo "[vast_setup][remote] Using uv-managed Python environment"\n'
  REMOTE_CMD+=$'else\n'
