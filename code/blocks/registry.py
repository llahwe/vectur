    ...
    "vectur": (
        ...
        lambda kwargs: (
            lambda dim: make_vectur_block(
                ...
                clip_delta_q=kwargs["clip_delta_q"] if "clip_delta_q" in kwargs else 5.0,
                clip_delta_theta=kwargs["clip_delta_theta"] if "clip_delta_theta" in kwargs else 1.0,
                clip_delta_w=kwargs["clip_delta_w"] if "clip_delta_w" in kwargs else 2.0,
                clip_delta_t=kwargs["clip_delta_t"] if "clip_delta_t" in kwargs else 5.0,
                clip_tape=kwargs["clip_tape"] if "clip_tape" in kwargs else 10.0,
                clip_w=kwargs["clip_w"] if "clip_w" in kwargs else 10.0,
                normalize_q=bool(kwargs.get("normalize_q", False)),
            )
        ),
    ),
    "vecstur": (
